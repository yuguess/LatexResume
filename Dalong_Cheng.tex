...
\resheading{{Skills}}
	\begin{description}
		\item[Languages:] { \footnotesize C/JAVA, \href{http://www.latex-project.org/}{\LaTeX}, MATLAB}
		\item[Technical Strength:] { \footnotesize System-level programming, Linux server administration}
...
\resheading{Work and Project Experience}
	\begin{itemize}
        \item 
            ...
            { \footnotesize
				\begin{itemize}
                    \resitem{A solid kernel with 28 basic system call}
					\resitem{A efficient memory management system}
					\resitem{A solid synchronization mechniasm}
					\resitem{A multi-thread library build upon the kernel}
				\end{itemize}
			}
		\item 
			\ressubheading{Sun Microsystem}{Beijing, China}{Software Engineer}{Sep.2010 -- Jul.2011}
            { \footnotesize
				\begin{itemize}
					\resitem{Do some system level programming for Solaris system}
					\resitem{Administrate Sun Solaris servers in Lab}
					\resitem{Setup and develop patches for Bugzilla for internal use}
				\end{itemize}
			}
		\item
